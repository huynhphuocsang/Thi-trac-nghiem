...
//1 la chuoi rong; 0 la chuoi binh thuong
...
//ham nay kiem tra ma sinh vien lieu co ton tai hay chua
//trung y tuong voi ham TimKiemSV_DSLOP dong  254
...
// Tim kiem lop
//nhap malop, neu ton tai tra ve vi tri lop, nguoc lai tra ve -1
...
//Luu lop(ten, ma) va danh sach sinh vien trong lop
...
//them diem thi vao cuoi danh sach 
...
//Ham kiem tra xem mamh co bi trung trong danh sach, da co san hay chua, co tra ve "false"
...
int nhapMONHOC(MONHOC &mh)
{
	...
	{ //Bat loi truong hop nhap vao la chuoi rong.
		...
		ShowCur(1);
		getline(cin, mh.MAMH);
		if (kiemTraChuoi(mh.MAMH) == 1)//chuoi rong
		{
			gotoxy(0, 20);
			ShowCur(0);
...
//them mon hoc moi theo mon hoc duoc truyen vao
...
int Sua_MonHoc(LISTMONHOC &dsmh)
{
	...
	while (kiemTraChuoi(mamonhocmoi) == 1)//rong
	{
		gotoxy(0, 20);
		ShowCur(0);
		cout << "Ban co muon thoat (y/n)";
...
//tim cau hoi theo ma id x truyen vao
...
//kiem tra id cau hoi lieu co trung voi gia tri chua trong mang nodes cua dethi hay khong
//neu trung tra ve false, neu khongo trung tra ve true
...
//them mot cau hoi moi vao de thi
...
{ //Ham them mot cau hoi vao danh sach DETHI da co san.
	...
	plist.nodes[plist.n] = id; 
	plist.n += 1;
	return 1;
}
//lay cau hoi bang cach duyet cay
...
//tao ra de thi theo so cau hoi do nguoi dung nhap vao
...
char menu_GV[16][100] =   {   "1. THEM LOP MOI                      ",
							  "2. IN DS LOP                         ",
...
//hien thi cau tra loi cua thi sinh
...
//kiem tra sinh vien co thi mon hoc nay hay khong? 
...
//lay diem cua sinh vien theo ma mh 
				...
				{ //Neu co nhan phim
					...
					switch (da)
					{
						...
						{ //Nhan enter 
							...
							gotoxy(0, 19);
							ShowCur(0);
							cout << "Dap an ban chon: " << dapan[cauthu]; 
							break;
						}
						case 'N': 
						case 'n':
						...
						{ //Nhan phim mui ten
							...
							switch (da)
							{
								...
								{ //Phim len di chuyen thanh sang len
									InCauTraLoi(ch->info, luachon, false); 
									if (luachon == 0)
									{
										luachon = 3;
									}
									...
									InCauTraLoi(ch->info, luachon, true); 
									break;
								}
								case 77:
...
//Xuat danh sach diem thi theo lop
...
//ham thiet ke menu khi dang nhap vao bang tai khoan cua giang vien
...
//thiet ke menu dang nhap ban dau
